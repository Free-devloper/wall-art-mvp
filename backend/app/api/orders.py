...
async def create_order(order_req: OrderCreate, db: AsyncSession = Depends(get_db), user: ClerkUser = Depends(get_clerk_user)):
...
async def regenerate_order(id: uuid.UUID, req: RegenerateRequest, db: AsyncSession = Depends(get_db)):
    ...
    await GenerationService.enqueue_generation(db, order.id, upload.id, order.theme_id, req.reason if hasattr(req, 'reason') else "")
...
async def create_checkout(id: uuid.UUID, db: AsyncSession = Depends(get_db)):
    ...
    if "placeholder" in settings.STRIPE_SECRET_KEY or settings.STRIPE_SECRET_KEY == "":
        ...
        return CheckoutSessionResponse(
            checkout_url=f"{settings.FRONTEND_URL}/order/{id}/confirmation"
        )
