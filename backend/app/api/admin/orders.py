    ...
    order_dict['generations'] = [
        {
            ...
            "preview_url": storage_service.generate_presigned_download_url(g.s3_key_preview) if g.s3_key_preview else None
...
async def get_production_file(id: uuid.UUID, db: AsyncSession = Depends(get_db), admin: AdminUser = Depends(get_current_admin)):
    order = await db.scalar(select(Order).where(Order.id == id))
    if not order:
        raise HTTPException(status_code=404, detail="Order not found")
...
async def admin_regenerate(id: uuid.UUID, db: AsyncSession = Depends(get_db), admin: AdminUser = Depends(get_current_admin)):
    order = await db.scalar(select(Order).where(Order.id == id))
    if not order:
        raise HTTPException(status_code=404, detail="Order not found")
