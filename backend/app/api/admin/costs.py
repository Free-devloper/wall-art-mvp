...
async def get_costs(db: AsyncSession = Depends(get_db), admin = Depends(get_current_admin)):
    ...
    today_result = await db.scalar(
        select(func.coalesce(func.sum(Generation.cost_usd), 0.0))
        .where(Generation.completed_at >= today_start)
    )
    
    # Total spent this month
    month_result = await db.scalar(
        select(func.coalesce(func.sum(Generation.cost_usd), 0.0))
        .where(Generation.completed_at >= month_start)
    )
    
    # Average cost per generation
    avg_result = await db.scalar(
        select(func.coalesce(func.avg(Generation.cost_usd), 0.0))
    )
    ...
    daily_query = await db.execute(
        select(
            cast(Generation.completed_at, Date).label("day"),
            func.sum(Generation.cost_usd).label("total")
        )
