...
class GenerationService:
    ...
    async def enqueue_generation(
        db: AsyncSession, order_id: uuid.UUID, upload_id: uuid.UUID, theme_id: uuid.UUID, instructions: str | None
    ) -> uuid.UUID:
    ...
    async def get_generation_status(db: AsyncSession, order_id: uuid.UUID) -> GenerationStatusResponse:
        ...
        result = await db.execute(
            select(Generation).where(Generation.order_id == order_id).order_by(Generation.created_at.desc()).limit(1)
        )
        ...
        count_res = await db.execute(select(func.count(Generation.id)).where(Generation.order_id == order_id))
        ...
        return GenerationStatusResponse(
            status=generation.status, preview_url=preview_url, remaining_regenerations=remaining
        )
